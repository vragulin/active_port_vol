...
COV_MAT_FREQ = WEEKLY
AVERAGE_VARS = True  # Whether to average variances or volatilities
...
def an_port_set(ports: dict, inp: dict, method: int = MCAP_W, scale_factor: float = 1) -> dict:
	...
	if AVERAGE_VARS:
		...
		avg_vol = np.mean(np.sqrt(np.maximum(vars, 0)))
...
if __name__ == "__main__":
	...
	res = an_port_set(ports[EQ_P], inp, method=MCAP_W)
