...
class Utility:
    ...
    def expUtil(self, w_scenarios, probs=None):
        ...
        if probs:
            raise NotImplementedError('Probabilities not implemented yet')

        w = w_scenarios
        n = len(w)
        return sum(self.u_of_wealth(v) for v in w) / n

    def ce_wealth(self, w_scenarios, probs=None):
        ...
        w = w_scenarios
        n = len(w)
        return self.implied_wealth(self.expUtil(w, probs))
